...
// ✅ 改良版 CORS 設定（允許 Render、Vercel、本地端）
app.use(
  cors({
    origin: [
      'http://localhost:3000',
...
${inventory.map(
  (i) =>
    `- ${i.name}：剩 ${i.remaining}%｜耐放約 ${i.shelfLife} 天｜到期日 ${
      i.expiry || '未填'
    }｜單價約 $${i.price}`
).join('\n')}

請依照下面原則回答：
1. 若問題是規劃菜單或食譜，請直接根據庫存給出具體建議（列出 3～5 道菜）。
2. 優先使用「剩餘量低或耐放天數少」的食材。
3. 回答要簡短實用，不要廢話、不要客套、不要反問問題。
`;
...
app.post('/api/smart-suggest', async (req, res) => {
  ...
  try {
    ...
    const avgDays = Math.round(
      inventory.reduce((a, b) => a + b.averageDays, 0) / (inventory.length || 1)
    );
    const totalValue = inventory.reduce((sum, i) => sum + i.price, 0);

    if (!genAI) {
      return res.json({
        answer: '⚠️ 尚未連上 Gemini，請根據剩餘量與保鮮期自行安排料理優先順序。',
        urgent,
        totalValue,
        avgDays,
      });
...
【庫存狀況】
${inventory.map(
  (i) => `- ${i.name}：剩${i.remaining}%｜耐放${i.shelfLife}天｜價格$${i.price}`
).join('\n')}

請輸出以下三部分（保持標題一致）：
📅 一週菜單建議（以 Markdown 表格呈現）
🧾 建議採購清單（含數量與單位）
💡 保存與料理提醒（3 行以內）
`;
    ...
    res.json({ answer, urgent, totalValue, avgDays });
  } catch (err) {
    console.error('❌ /api/smart-suggest 錯誤:', err.message);
    res.json({
      answer: '智慧菜單規劃暫時出錯，但庫存資料仍可用，請稍後再試。',
      urgent: [],
      totalValue: 0,
      avgDays: 0,
    });
